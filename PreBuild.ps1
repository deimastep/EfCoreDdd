...
function Update-AssemblyInfoVersionFiles
{
  ...
  (
    ...
    $AllVersionFiles = Get-ChildItem $SrcPath SolutionInfo.cs -recurse
    ...
    $newVersion = $assemblyFileVersion = "$major.$minor.$julianDate.$buildIncrementalNumber"
...
$newVersion = Update-AssemblyInfoVersionFiles $productVersion
Write-Verbose -Verbose "##vso[build.updatebuildnumber]$newVersion"
